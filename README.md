...
| LibraryManagement | OOP: Book abstract parent + FictionBook/TextBook children, library menu | Done (all 8 menu cases implemented: add fiction/textbook, display all, search/remove by ID, calculate late fee, filter by borrow days, sort by title A-Z) |
| Lab01LMSPractice | OOP micro-step rebuild of a Library Management System (11-module ROADMAP.md, guided/mentored build) | In progress — Modules 1-3 done (Book parent + Novel/Textbook/Comic/Others subclasses + Member class, all in `src/schema/`); Module 4 (`Borrowing` business logic) and the `LAB01` controller (Modules 5-8) plus file I/O persistence (Modules 9-11) not started yet |
